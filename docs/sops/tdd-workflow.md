...
When: Writing new code in `infra/` (new features, bugfixes, behavioral changes).
...
- Test infrastructure uses **vitest** (see `infra/scheduler/vitest.config.ts` for reference)
...
```bash
cd infra/scheduler && npm test -- --run path/to/test.test.ts
```
...
Check: Every code change in `infra/` has a corresponding test file with passing tests.
...
33% of TDD violations originate from Slack diagnosis urgency — the "user is waiting" pressure bypasses test discipline. Writing the test first takes ~2 minutes and prevents regressions that take hours to debug.
