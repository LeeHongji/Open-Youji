/** Slack Bot for Youji scheduler. Socket Mode for multi-channel interaction.
...
export async function startSlackBot(opts: {
  ...
  // Handle /youji slash command (ADR 0033)
  app.command("/youji", async ({ command, ack, respond }) => {
    await ack();
    const result = await handleYoujiCommand({
      text: command.text ?? "",
...
export function startupMessage(opts: { totalJobs: number; enabledJobs: number; nextRun: string; backend: string }): string {
  return (
    `:rocket: *Youji scheduler started*\n` +
...
export async function notifySessionComplete(
  ...
  const fallback = `Youji session ${result.ok ? "completed" : "failed"}: ${job.name}`;
...
async function publishHomeTab(userId: string): Promise<void> {
  ...
  const blocks = [
    { type: "header", text: { type: "plain_text", text: "Youji scheduler" } },
...
// --- /youji slash command handler (ADR 0033) ---

export interface YoujiCommandInput {
  text: string;
  userId: string;
  channelId: string;
}

export interface YoujiCommandResult {
  text: string;
}

/** Parse and execute /youji slash commands. Exported for testing. */
export async function handleYoujiCommand(input: YoujiCommandInput): Promise<YoujiCommandResult> {
  ...
  if (!subcommand || subcommand === "help") {
    return {
      text:
        `:bulb: */youji* commands:\n` +
        `• \`/youji mode dev\` — set this channel to dev mode (full access)\n` +
        `• \`/youji mode chat\` — set this channel to chat mode (read-only Q&A)\n` +
        `• \`/youji mode off\` — remove this channel from Youji\n` +
        `• \`/youji max-turns N\` — limit bot replies per thread to N turns\n` +
        `• \`/youji max-turns off\` — remove the turn limit\n` +
        `• \`/youji backend <claude|cursor|opencode|auto>\` — switch agent backend\n` +
        `• \`/youji backend reset\` — reset to default (auto with env fallback)\n` +
        `• \`/youji status\` — show this channel's current mode and backend\n` +
        `• \`/youji help\` — this message\n\n` +
  ...
  if (subcommand === "max-turns") {
    if (!isDesignatedUser(input.userId)) {
      return { text: `:no_entry: Only the designated Youji operator can change turn limits.` };
    }
    ...
    if (!valueArg) {
      ...
      return { text: `:information_source: No turn limit set for this channel. Use \`/youji max-turns N\` to set one.` };
    }
    ...
    if (isNaN(n) || n <= 0) {
      return { text: `:warning: Invalid turn limit \`${valueArg}\`. Provide a positive number (e.g., \`/youji max-turns 10\`).` };
    }
  ...
  if (subcommand === "status") {
    ...
    return { text: `:information_source: This channel has no mode configured.${turnsInfo}${backendInfo} Use \`/youji mode dev\` or \`/youji mode chat\` to set one.` };
  }

  if (subcommand === "backend") {
    if (!isDesignatedUser(input.userId)) {
      return { text: `:no_entry: Only the designated Youji operator can change the backend.` };
    }
    ...
    if (!backendArg) {
      ...
      const persisted = getBackendPreference();
      if (persisted) {
        return { text: `:information_source: Current backend: *${current}* (persisted). Use \`/youji backend <claude|cursor|opencode|auto>\` to change.` };
      }
      return { text: `:information_source: Current backend: *${current}* (env default). Use \`/youji backend <claude|cursor|opencode|auto>\` to persist a preference.` };
    }
  ...
  if (subcommand === "mode") {
    if (!isDesignatedUser(input.userId)) {
      return { text: `:no_entry: Only the designated Youji operator can change channel modes.` };
    }
    ...
    if (modeArg === "off" || modeArg === "remove" || modeArg === "none") {
      ...
      if (removed) {
        return { text: `:white_check_mark: Channel mode removed. Youji will no longer respond in this channel.` };
      }
  ...
  return { text: `:warning: Unknown command \`${subcommand}\`. Try \`/youji help\`.` };
}
...
export async function handleBotChannelJoin(
  ...
  try {
    await client.chat.postMessage({
      ...
      text: "Thanks for the invite! However, only the Youji operator can add me to channels. Please ask them to invite me if you'd like me here.",
    });
  } catch {
