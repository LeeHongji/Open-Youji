/** Natural language chat interface for Youji Slack bot.
...
/** Detect burst mode requests like "run burst mode on youji-work-cycle".
...
export function detectBurstRequest(message: string): { job: string; maxSessions: number; maxCost: number; autofix: boolean } | null {
  ...
  // Pattern 1: command verb + [youji] burst [mode] [on/for <rest>]
  const verbMatch = lower.match(
    /^(?:run|start|activate|launch|trigger|do)\s+(?:youji\s+)?burst\s*(?:mode)?(?:\s+(?:on|for)\s+(?:the\s+)?)?(.+)?/s,
  );

  // Pattern 2: bare "burst [mode]" — only valid if followed by "on/for <job>" or nothing
  const bareMatch = !verbMatch && lower.match(
    /^(?:youji\s+)?burst\s*(?:mode)?(?:\s+(?:on|for)\s+(?:the\s+)?(.+))?$/s,
  );
  ...
  const jobName = jobMatch?.[1] || "youji-work-cycle";
