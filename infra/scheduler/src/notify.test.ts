...
describe("parsePendingItems", () => {
  ...
  it("parses burst-specific fields from pending items", () => {
    const content = [
      "## Pending",
      "",
      "### 2026-02-26 — Burst mode: youji-work-cycle",
      "Project: youji",
      "Type: burst",
      "Request: Run a burst of autonomous sessions on the youji-work-cycle job.",
    ...
    expect(items).toHaveLength(1);
    expect(items[0].type).toBe("burst");
    expect(items[0].job).toBe("youji-work-cycle");
...
describe("resolveApproval — burst field preservation", () => {
  ...
  it("preserves burst fields when resolving a burst approval", async () => {
    ...
    expect(resolvedItems[0].job).toBe("youji-work-cycle");
