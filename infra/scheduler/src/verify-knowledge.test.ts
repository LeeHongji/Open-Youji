...
describe("PROJECT_README_RE", () => {
  it("matches top-level project READMEs", () => {
    expect(PROJECT_README_RE.test("projects/youji/README.md")).toBe(true);
    expect(PROJECT_README_RE.test("projects/sample-project/README.md")).toBe(true);
  });

  it("matches nested subdirectory READMEs", () => {
    expect(PROJECT_README_RE.test("projects/youji/log/README.md")).toBe(true);
  });
  ...
  it("rejects non-README files under projects", () => {
    expect(PROJECT_README_RE.test("projects/youji/EXPERIMENT.md")).toBe(false);
...
describe("EXPERIMENT_MD_RE", () => {
  it("matches EXPERIMENT.md files under projects/*/experiments/", () => {
    expect(EXPERIMENT_MD_RE.test("projects/youji/experiments/test/EXPERIMENT.md")).toBe(true);
  ...
  it("rejects EXPERIMENT.md files outside experiments directory", () => {
    expect(EXPERIMENT_MD_RE.test("projects/youji/EXPERIMENT.md")).toBe(false);
    expect(EXPERIMENT_MD_RE.test("projects/youji/results/EXPERIMENT.md")).toBe(false);
  });

  it("rejects non-EXPERIMENT.md files under experiments directory", () => {
    expect(EXPERIMENT_MD_RE.test("projects/youji/experiments/test/config.yaml")).toBe(false);
    expect(EXPERIMENT_MD_RE.test("projects/youji/experiments/test/results.csv")).toBe(false);
  });
...
describe("parseKnowledgeFromDiff", () => {
  ...
  it("counts numbered findings in EXPERIMENT.md diffs", () => {
    const diff = `diff --git a/projects/youji/experiments/test/EXPERIMENT.md b/projects/youji/experiments/test/EXPERIMENT.md
--- a/projects/youji/experiments/test/EXPERIMENT.md
+++ b/projects/youji/experiments/test/EXPERIMENT.md
@@ -10,0 +11,3 @@
...
`;
    const result = parseKnowledgeFromDiff(diff, [
      "projects/youji/experiments/test/EXPERIMENT.md",
    ]);
...
`;
  ...
  it("counts new unchecked tasks in TASKS.md diffs", () => {
    const diff = `diff --git a/projects/youji/TASKS.md b/projects/youji/TASKS.md
--- a/projects/youji/TASKS.md
+++ b/projects/youji/TASKS.md
@@ -10,0 +11,2 @@
+- [ ] New task one [fleet-eligible]
+- [ ] New task two [requires-opus]
`;
    const result = parseKnowledgeFromDiff(diff, [
      "projects/youji/TASKS.md",
    ]);
...
`;
  ...
  it("does not count tasks in non-TASKS.md files", () => {
    const diff = `diff --git a/projects/youji/README.md b/projects/youji/README.md
--- a/projects/youji/README.md
+++ b/projects/youji/README.md
@@ -10,0 +11,2 @@
+- [ ] This looks like a task but is in README
+- [ ] Another fake task
`;
    const result = parseKnowledgeFromDiff(diff, [
      "projects/youji/README.md",
    ]);
...
`;
  ...
  it("counts bugfix experiments with verification sections", () => {
    const diff = `diff --git a/projects/youji/experiments/fix-race-condition/EXPERIMENT.md b/projects/youji/experiments/fix-race-condition/EXPERIMENT.md
@@ -3 +3 @@
-status: planned
+status: completed
@@ -10,0 +11,5 @@
...
`;
    const result = parseKnowledgeFromDiff(diff, [
      "projects/youji/experiments/fix-race-condition/EXPERIMENT.md",
    ]);
...
`;
  ...
  it("correctly counts a typical session with mixed output types", () => {
    const diff = `diff --git a/projects/youji/experiments/spot-check/EXPERIMENT.md b/projects/youji/experiments/spot-check/EXPERIMENT.md
@@ -3 +3 @@
-status: planned
+status: completed
@@ -20,0 +21,3 @@
...
diff --git a/projects/youji/README.md b/projects/youji/README.md
@@ -15,0 +15,6 @@
...
`;
    const changedFiles = [
      "projects/youji/experiments/spot-check/EXPERIMENT.md",
      "projects/youji/README.md",
      "projects/youji/analysis/validation-coverage.md",
...
-Old line 2
`;
    const result = parseKnowledgeFromDiff(diff, ["projects/youji/README.md"]);
    expect(result.newExperimentFindings).toBe(0);
    expect(result.logEntryFindings).toBe(0);
  });
  ...
  it("counts pattern doc changes as compound actions", () => {
    const diff = "";
    const result = parseKnowledgeFromDiff(diff, [
      "projects/youji/patterns/skills-architecture.md",
      "projects/youji/patterns/autonomous-execution.md",
    ], new Set());
    expect(result.compoundActions).toBe(2);
  });
...
`;
  ...
  it("counts new feedback files", () => {
    const diff = "";
    const result = parseKnowledgeFromDiff(diff, [
      "projects/youji/feedback/feedback-orient-weight.md",
      "projects/youji/feedback/feedback-token-efficiency.md",
    ], new Set([
      "projects/youji/feedback/feedback-orient-weight.md",
      "projects/youji/feedback/feedback-token-efficiency.md",
    ]));
  ...
  it("counts new diagnosis files", () => {
    const diff = "";
    const result = parseKnowledgeFromDiff(diff, [
      "projects/youji/diagnosis/diagnosis-flash-240-crash.md",
    ], new Set([
      "projects/youji/diagnosis/diagnosis-flash-240-crash.md",
    ]));
  ...
  it("counts new postmortem files", () => {
    const diff = "";
    const result = parseKnowledgeFromDiff(diff, [
      "projects/youji/postmortem/postmortem-hallucination-2026-02-19.md",
    ], new Set([
      "projects/youji/postmortem/postmortem-hallucination-2026-02-19.md",
    ]));
    expect(result.diagnosesCompleted).toBe(1);
  ...
  it("counts both diagnosis and postmortem files together", () => {
    const diff = "";
    const result = parseKnowledgeFromDiff(diff, [
      "projects/youji/diagnosis/diagnosis-budget-issue.md",
  ...
  it("does not count modified diagnosis files as new", () => {
    const diff = "";
    const result = parseKnowledgeFromDiff(diff, [
      "projects/youji/diagnosis/diagnosis-old-issue.md",
    ], new Set());
    expect(result.diagnosesCompleted).toBe(0);
  });
});
...
describe("parseCrossProjectMetrics", () => {
  it("detects projects touched from changed files", () => {
    ...
    expect([...result.projectsTouched].sort()).toEqual(["youji", "sample-project"]);
  ...
  it("counts findings per project from EXPERIMENT.md diffs", () => {
    const diff = [
      "diff --git a/projects/youji/experiments/test/EXPERIMENT.md b/projects/youji/experiments/test/EXPERIMENT.md",
      "+1. First finding",
      "+2. Second finding",
    ...
    expect(result.findingsPerProject).toEqual({ youji: 2, "sample-project": 1 });
  });
  ...
  it("detects cross-project references in added lines", () => {
    ...
    const result = parseCrossProjectMetrics(diff, ["projects/youji/README.md"]);
  ...
  it("does not count within-project references as cross-project", () => {
    const diff = [
      "diff --git a/projects/youji/README.md b/projects/youji/README.md",
      "+See `projects/youji/experiments/test/EXPERIMENT.md` for details.",
    ].join("\n");
  ...
  it("handles multiple cross-project references on one line", () => {
    const diff = [
      "diff --git a/projects/youji/analysis/test.md b/projects/youji/analysis/test.md",
      "+Compare projects/sample-project/data.csv with projects/bench-project/results.csv",
    ].join("\n");

    const result = parseCrossProjectMetrics(diff, ["projects/youji/analysis/test.md"]);
    expect(result.crossProjectRefs).toBe(2);
  });
  ...
  it("combines findings from experiment and readme in same project", () => {
    ...
    expect(result.findingsPerProject).toEqual({ youji: 2 });
  });
});
...
describe("parseQualityAuditMetrics", () => {
  ...
  it("detects audit skill mentions in README log entries", () => {
    ...
    const result = parseQualityAuditMetrics(diff, ["projects/youji/README.md"]);
  ...
  it("does not count new EXPERIMENT.md files as audited", () => {
    const diff = [
      "diff --git a/projects/youji/experiments/new-exp/EXPERIMENT.md b/projects/youji/experiments/new-exp/EXPERIMENT.md",
      "+---",
      "+id: new-exp",
      "+status: planned",
      "+---",
    ...
    const changedFiles = ["projects/youji/experiments/new-exp/EXPERIMENT.md"];
    const newFiles = new Set(["projects/youji/experiments/new-exp/EXPERIMENT.md"]);
  ...
  it("does not count findings in new EXPERIMENT.md files", () => {
    const diff = [
      "diff --git a/projects/youji/experiments/new/EXPERIMENT.md b/projects/youji/experiments/new/EXPERIMENT.md",
      "+1. Finding in a new experiment.",
      "+This is incorrect but it's in a new file.",
    ].join("\n");

    const changedFiles = ["projects/youji/experiments/new/EXPERIMENT.md"];
    const newFiles = new Set(["projects/youji/experiments/new/EXPERIMENT.md"]);
    const result = parseQualityAuditMetrics(diff, changedFiles, newFiles);
    expect(result.auditFindings).toBe(0);
    expect(result.experimentsAudited).toBe(0);
  });
  ...
  it("handles a complete audit session with mixed signals", () => {
    const diff = [
      ...
      "diff --git a/projects/youji/experiments/exp-a/EXPERIMENT.md b/projects/youji/experiments/exp-a/EXPERIMENT.md",
      "+1. Missing provenance on claim about 95% accuracy.",
      "diff --git a/projects/youji/experiments/exp-b/EXPERIMENT.md b/projects/youji/experiments/exp-b/EXPERIMENT.md",
      "+Corrected the row count from 100 to 97.",
      "diff --git a/projects/youji/experiments/exp-c/EXPERIMENT.md b/projects/youji/experiments/exp-c/EXPERIMENT.md",
      "+No issues found.",
    ].join("\n");

    const changedFiles = [
      "projects/youji/README.md",
      "projects/youji/experiments/exp-a/EXPERIMENT.md",
      "projects/youji/experiments/exp-b/EXPERIMENT.md",
      "projects/youji/experiments/exp-c/EXPERIMENT.md",
    ];
    const result = parseQualityAuditMetrics(diff, changedFiles, new Set());
    expect(result.auditSkillsInvoked).toBe(2);
  ...
  it("does not detect audit skills in non-README files", () => {
    const diff = [
      "diff --git a/projects/youji/TASKS.md b/projects/youji/TASKS.md",
      "+- [ ] Run /review on completed experiments",
    ].join("\n");

    const result = parseQualityAuditMetrics(diff, ["projects/youji/TASKS.md"]);
    expect(result.auditSkillsInvoked).toBe(0);
  });
