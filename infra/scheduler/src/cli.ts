#!/usr/bin/env node
/** CLI for the youji scheduler. Manages cron jobs and runs the scheduler daemon. */
...
//   1. infra/.env        — common vars shared across all youji infra (Databricks, AWS, etc.)
...
const HELP = `
youji — Cron scheduler for autonomous agent sessions
...
Check-health options:
  ...
  --state-file <path>       State file for tracking consecutive failures (default: /tmp/youji-health-state.json)
...
async function cmdCheckHealth(args: string[]): Promise<void> {
  ...
  const stateFile = opts["state-file"] ?? "/tmp/youji-health-state.json";
  const notify = args.includes("--notify");
  ...
  if (isHealthy) {
    ...
    if (state.consecutiveFailures >= 2 && !state.alertSent && notify && slackImpl.isConfigured()) {
      ...
      const msg = `:rotating_light: *Scheduler health check failed*\n` +
        ...
        `Time: ${now}\n\n` +
        `_Run \`pm2 logs youji\` or check \`systemctl status youji\` for details._`;
      await slackImpl.dm(msg);
