...
description: "Lightweight session-end learning capture when full compound is unnecessary"
---

# compound-simple

Lightweight compound skill for quick session-end embedding in <3 turns.

## When to use

At the end of a work session when full compound is unnecessary — short sessions, simple tasks, or when full compound recently ran.
...
1. **Non-obvious fact discovered?** (API quirk, hidden constraint, unclear convention)
   -> Update relevant file (CLAUDE.md, skill, or project file)
...
- Do NOT run full /compound skill — too complex for quick sessions
- Do NOT start new work — compound embeds learnings only
- Do NOT scan cross-session patterns
