...
argument-hint: "<topic> [project path]"
---
...
- The topic directly
- The topic combined with relevant technical terms (e.g., "LLM judge evaluation metrics", "agent benchmark")
...
Save verified notes to `literature/` within the project directory (create it if needed), or to `docs/literature/` if not project-specific. Follow the literature note schema in CLAUDE.md.
...
1. **Identify the action**: What does this paper enable or suggest?
...
3. **If no tasks are warranted** (literature fully answers questions with no follow-up needed), state this explicitly: "No follow-up tasks: literature fully addressed the inquiry."
...
Commit message: `lit-review: <topic> — <N> papers reviewed`
